...
const months = ['2022-07', '2022-08', '2022-09'];
...
async function get(prs, token) {
  ...
  const reviews = [];
  for (const pr of prs) {
    const r = await fetch(`${pr.url}/reviews`, {method:'GET', headers: requestHeaders});
...
const quarterlyPRs = prs.filter(pr => {
  if (pr.merged_at) {
    return months.includes(pr.merged_at.slice(0, 7));
  }
  return false;
});

get(quarterlyPRs, token);
